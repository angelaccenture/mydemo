let _taxonomy;
    ...
    function initCategories() {
      CATEGORIES.push(_taxonomy.getCategoryTitle(_taxonomy.CATEGORIES));
      CATEGORIES.push(_taxonomy.getCategoryTitle(_taxonomy.PRODUCTS));
      CATEGORIES.push(_taxonomy.getCategoryTitle(_taxonomy.INDUSTRIES));
    ...
    function addByName(name, cat) {
      ...
      if (item) {
        selected.push({
          name: item.name, 
          category: cat || _taxonomy.CATEGORIES
        });
        displaySelected();
      }
    }
    ...
    function handlePaste(event) {
      ...
      if (paste) {
        ...
        if (topics.length === 0 && products.length === 0) {
          // new format
          topics = paste.split(', ');
        }
